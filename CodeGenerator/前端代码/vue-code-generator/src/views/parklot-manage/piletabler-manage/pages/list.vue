<template>
  <div class="app-container">
    ...
    <div style="text-align:right;" class="area">
      ...
      <el-button type="success" size="small" @click="editRecord('edit')">
    ...
    <div class="area">
      ...
      >
        ...
        <el-table-column width="100" label="登记日期" property="RegisterDate" align="center" :formatter="formatter" />
        ...
        <el-table-column width="200" label="工号" property="EmpNo" align="center" />
        <el-table-column label="姓名" property="EName" align="center" />
        <el-table-column width="250" label="操作" property="Guid" align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="editRecord('edit' , scope.row)">修改</el-button>
            <el-button type="warning" size="mini" @click="editRecord('delete' , scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getlist: function(pager) {
      ...
      if (!vm.query.RegisterDate) {
        vm.$delete(vm.query, 'RegisterDate')
      }
    ...
    //  编辑数据
    editRecord: function(type, row) {
      if (type === 'edit') {
        if (row) {
          this.init.dialog.id = row.Guid
          this.init.dialog.title = '编辑抄表工'
        } else {
          this.init.dialog.title = '新增抄表工'
        }
        ...
        this.$confirm('您确定要删除这条数据吗?', '提示', {
          ...
          communicate
            .SetData('MRWorkerInfo', { Guid: row.Guid }, row.Guid)
            .then(data => {
              if (data == 0) {
                this.refresh(true)
                this.$message({
      ...
      communicate
        ...
        .then(response => {
          if (response[vm.init.table.listnode]) {
            ...
            data.map(item => {
              //  日期格式化
              item.RegisterDate = this.parseTime(new Date(item.RegisterDate), '{y}-{m}-{d}')
            })
            excelhelper.export('充电站抄表工信息表', headers, columns, data)
          } else {
            vm.msgError('无数据')
          }
          vm.unloading(myload)
