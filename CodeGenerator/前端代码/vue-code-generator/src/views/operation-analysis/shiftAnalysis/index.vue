  ...
  <div class="app-container">
    ...
    <div class="area">
      ...
      >
        ...
        <el-table-column prop="PlamNum" width="180" label="计划班次" :resizable="true" align="center" />
        <el-table-column prop="ActuallyNum" width="180" label="实际班次" :resizable="true" align="center" />
        <el-table-column prop="StopNum" width="180" label="停运班次" :resizable="true" align="center">
          <template slot-scope="scope">
            <div style="color:red;text-decoration:underline;cursor:pointer;" @click="clickStop(scope.row)">{{ scope.row.StopNum }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="HalfStopNum" width="180" label="中途停运班次" :resizable="true" align="center">
          <template slot-scope="scope">
            <div style="color:red;text-decoration:underline;cursor:pointer;" @click="clickHalfStop(scope.row)">{{ scope.row.HalfStopNum }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="ErrorNum" width="180" label="异常班次" :resizable="true" align="center">
          <template slot-scope="scope">
            <div style="color:red;text-decoration:underline;cursor:pointer;" @click="clickError(scope.row)">{{ scope.row.ErrorNum }}</div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 添加或修改参数配置对话框 -->
    <el-dialog title="班次详情" :visible.sync="open" width="1000px" height="500px">
      <el-table
        :ref="init.table.key"
        :key="init.table.key"
      ...
      >
        ...
        <el-table-column prop="BusNo" width="180" label="车辆自编号" :resizable="true" align="center" />
        <el-table-column prop="LName" width="180" label="线路番号" :resizable="true" align="center" />
        <el-table-column prop="EmpNo" width="180" label="员工工号" :resizable="true" align="center" />
        <el-table-column prop="EndTime" width="180" label="班次时间" :resizable="true" align="center" />
      </el-table>
...
export default {
    ...
    return {
      ...
      init: {
        ...
        table: {
          ...
          key: 'table',
          emptytext: '暂无数据',
          datatype: 'QueryDailyShiftExectue',
          datadetailtype: 'QueryDailyShiftExectueDetail',
  ...
  methods: {
    ...
    clickError(row) {
      ...
      this.detailQuery.BSType = 3
      this.getDetail()
    },
    clickStop(row) {
      this.getDetilQuery(row)
      this.detailQuery.BSType = 1
      this.getDetail()
    },
    clickHalfStop(row) {
      this.getDetilQuery(row)
      this.detailQuery.BSType = 2
      this.getDetail()
    },
    getDetilQuery(row) {
      this.open = true
      ...
      if (this.listQuery.LName) {
        this.detailQuery.LName = this.listQuery.LName
      }
      if (this.listQuery.lDirection) {
        this.detailQuery.lDirection = this.listQuery.lDirection
      }
      if (row.CompName) {
    ...
    getDetail: function() {
      ...
      if (!vm.detailQuery.LName) {
        vm.$delete(vm.detailQuery, 'LName')
      }
      if (!vm.detailQuery.lDirection) {
        vm.$delete(vm.detailQuery, 'lDirection')
      }
      communicate
      ...
      communicate
        ...
        .then(response => {
          if (response[vm.init.table.listnode]) {
            ...
            excelhelper.export('班次执行统计', headers, columns, data)
          } else {
            vm.msgError('无数据')
          }
          vm.unloading(myload)
