  ...
  <div class="app-container">
    ...
    <div class="area">
      <el-tabs v-model="listQuery.SortMethod" type="card" @tab-click="handleClick">
        ...
        <el-tab-pane label="场站" name="TName" />
      </el-tabs>
      <el-table
        :ref="init.table.key"
        :key="init.table.key"
      ...
      >
        ...
        <el-table-column
          ...
          align="center"
        />
        <el-table-column v-if="listQuery.SortMethod =='LName'" prop="LName" width="180" label="线路" :resizable="true" align="center" />
        <el-table-column v-if="listQuery.SortMethod =='LName'" prop="lDirection" label="方向" :resizable="true" align="center" />
        <el-table-column v-if="listQuery.SortMethod =='TName'" prop="TName" label="场站" :resizable="true" align="center" />
        <el-table-column prop="TheoreticalNum" width="180" label="理论数量" :resizable="true" align="center" />
        <el-table-column prop="Num" width="180" :label="$t('detail.count')" :resizable="true" align="center">
          <template slot-scope="scope">
            <div v-if="listQuery.SortMethod =='LName'" style="color:red;text-decoration:underline;cursor:pointer;" @click="clickDetail(scope.row)">{{ scope.row.Num }}</div>
            <div v-if="listQuery.SortMethod !='LName'">{{ scope.row.Num }}</div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="进出场详情" :visible.sync="open" width="1000px" height="500px">
      <el-table
        :ref="init.table.key"
        :key="init.table.key"
    ...
    return {
      ...
      init: {
        ...
        table: {
          ...
          key: 'table',
          emptytext: '暂无数据',
          datatype: 'QueryDailyStationSummary',
          datadetailtype: 'QueryDailyStationDetail',
      ...
      communicate
        ...
        .then(response => {
          if (response[vm.init.table.listnode]) {
            ...
            vm.unloading(myload)
            excelhelper.export('进出场信息统计', headers, columns, data)
          } else {
            vm.msgError('无数据')
          }
          vm.unloading(myload)
